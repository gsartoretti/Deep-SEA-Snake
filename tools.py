...
#    return (snakeCompl, snakeNom)
    return (amp.transpose(), spFreq.transpose(), snakeNom)

def nn_layer(sess, input_tensor, weights, biases, layer_name, act=tf.nn.relu):
    ...
    preactivate = np.matmul(input_tensor, weights) + biases
#    activations = act(preactivate, name='activation')
    activations = list( map(np.max, zip(preactivate[0], [0 for i in range(max(preactivate.shape))])) )
    return np.array(activations)

def get_NewNomParam_QN(snakeCompl, snakeNom, snakeConst, windows, tau_Applied, dt, t, worker, actions_list, sess):
    compliantQ = True
    steerQ = False
#    deterministicQ = True
    deterministicQ = False
    ...
    if compliantQ:
        for win in range(len):
            state               = np.array( [(t % serp_period) / serp_period, sigma_D[len+win], sigma_D[2*len+win], tau_Applied[win], tau_Applied[len + win]] ).transpose()
            state               =np.reshape(state,(1,5))
            action              = workers[0].AC.choose_action(state)
            sigma_D[len+win]   += actions_list[action][0]
            sigma_D[2*len+win] += actions_list[action][1]
...
def get_NewNomParam_A3C(snakeCompl, snakeNom, snakeConst, windows, tau_Applied, dt, t, worker, actions_list, sess):
    
    len               = windows.numWindows
    
    amp               = snakeNom.sigma_D[    len    : 2 * len]
    spFreq            = snakeNom.sigma_D[2 * len    : 3 * len]
    if any(spFreq <= 0.01):
        for i,f in enumerate(spFreq):
            if f <= 0.01:
                spFreq[i] = 0.01
    ...
    snakeCompl.amp      = amp.transpose() ## transpose is to put amp back into standard form
    snakeCompl.spFreq   = spFreq.transpose()
    snakeCompl.tmFreq   = snakeCompl.tmFreq + snakeConst.tmFreq * dt
...
def get_state(snakeConst, snakeCompl, windows):
